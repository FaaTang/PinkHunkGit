...
async function applySoftExclusive(
	...
	if (announce && context.workspaceState.get<boolean>(NOTIFIED_KEY) !== true) {
		...
		showTimedInfoMessage(
			'Pink Hunk Git soft exclusive: disabled built-in Git autofetch/autorefresh. Commit panel watchers plus a 1.5s active-repo status poll replace git.autorefresh. Toggle via setting “Soft Exclusive Git”.'
		);
	}
}
