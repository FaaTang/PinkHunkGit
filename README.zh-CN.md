...
- 软独占 Git（默认开）：在工作区关闭内置 `git.autofetch` / `git.autorefresh`，降低 `index.lock` 冲突，同时保持 `vscode.git` 启用以供本插件 API。可在设置 **Pink Hunk Git: Soft Exclusive Git** 或命令 **Toggle Soft Exclusive Git** 开关。Commit 面板可见时，除文件监视外还会约每 1.5 秒刷新当前仓库状态，避免本地改动延迟出现。另外会监视 `.git/HEAD`、`index` 与 `refs`，因此 IDEA 等外部客户端提交后会自动跟上，无需手动刷新
