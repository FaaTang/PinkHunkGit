...
- 仓库分支名后显示可拉取 / 可推送数量（`N↓` / `N↑`）；点击即可拉取该仓库或打开该仓库的 Push 对话框（支持多仓库）
...
- 软独占 Git（默认开）：在工作区关闭内置 `git.autofetch` / `git.autorefresh`，降低 `index.lock` 冲突，同时保持 `vscode.git` 启用以供本插件 API。可在设置 **Pink Hunk Git: Soft Exclusive Git** 或命令 **Toggle Soft Exclusive Git** 开关。Commit 面板会在文件变更后自行刷新；面板可见时还会周期性刷新当前仓库状态，避免新建未追踪文件漏显
