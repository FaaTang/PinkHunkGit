...
- Changes / Unversioned Files groups (IDEA-style); repositories collapsed by default (categories stay open); Unversioned / Ignored hide repositories with no files; Expand All / Collapse All apply to the selected group, repository, or folder — with a confirm dialog when none is selected
...
- In the Changes group, per-repository ahead/behind (`N↓` / `N↑`) after the branch name; click to pull that repo or open Push for it. Unversioned / Ignored omit these actions so count and branch stay aligned
- Push dialog (`Ctrl+Shift+K`): repositories with unpushed commits are checked by default; opening from a specific repo (`N↑`) or after Commit and Push keeps that narrower selection
...
- Soft exclusive Git (default on): disables built-in `git.autofetch` / `git.autorefresh` in the workspace to reduce `index.lock` races, while keeping `vscode.git` enabled for this extension’s API. Toggle via setting **Pink Hunk Git: Soft Exclusive Git** or command **Toggle Soft Exclusive Git**. While the Commit panel is visible, the extension status-refreshes the active repository about every 1.5 seconds (in addition to file watchers) so local changes are not delayed. It also triggers one silent upstream fetch immediately on startup, then fetches each repository’s upstream about every 3 minutes (VS Code `git.autofetchPeriod` default) so `N↓` / `N↑` stay current, and watches `.git/HEAD`, `index`, and `refs`, so commits from IDEA or other Git clients show up without a manual refresh
